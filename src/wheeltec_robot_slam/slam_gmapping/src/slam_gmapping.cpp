...
#define MAP_IDX(sx, i, j) ((sx) * (j) + (i))//作用：将二维栅格坐标(i,j)转换为ROS OccupancyGrid消息的一维数据数组索引（ROS地图用一维数组存储二维栅格）

using std::placeholders::_1;

SlamGmapping::SlamGmapping():
    Node("slam_gmapping"),      // 初始化父类rclcpp::Node，节点名称为"slam_gmapping"（ROS 2节点标识）
    scan_filter_sub_(nullptr),  // 激光数据订阅器（message_filters）初始化：空指针
    scan_filter_(nullptr),      // 激光数据TF过滤器（message_filters）初始化：空指针
    laser_count_(0),            // 激光帧计数器：初始为0（用于数据节流）
    transform_thread_(nullptr)  // TF发布线程：初始为0（后续在startLiveSlam()中创建）
{
...
//设置 SLAM 算法的默认参数（粒子滤波、激光匹配、运动模型、地图属性），为后续传感器初始化和核心处理提供参数基础。
...
//创建 SLAM 所需的 ROS 发布器、订阅器和过滤器，启动独立 TF 发布线程，搭建数据输入（激光订阅）和输出（地图 / TF 发布）的通道，标志实时 SLAM 流程启动。
...
//在独立线程中按固定周期触发 TF 变换发布，避免主线程（激光处理）阻塞，确保/tf话题稳定输出（供导航等节点使用）。
...
//通过 TF 缓冲区查询激光雷达在里程计坐标系下的 2D 位姿（x/y/yaw），为后续激光数据与里程计融合提供位姿基准。
