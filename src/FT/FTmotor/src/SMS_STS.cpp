...
int SMS_STS::isOverload(u8 ID,bool Enable)
{
	int en = Enable ? 1 : 0;
	return writeByte(ID,SMS_STS_UNLOAD,en);
}

int SMS_STS::protectedForce(u8 ID,u8 force)
{
	return writeByte(ID,SMS_STS_PROTECT_FORCE,force);
}
