...
int main(int argc, char **argv)
{
  ...
  blackboard_->set<std::chrono::milliseconds>("server_timeout", std::chrono::milliseconds(1000000)); 
  blackboard_->set<std::chrono::milliseconds>("wait_for_service_timeout", std::chrono::milliseconds(1000000));
  ...
  auto navigation_action_client_ = rclcpp_action::create_client<nav2_msgs::action::NavigateToPose>( node, "navigate_to_pose");
  navigation_action_client_->wait_for_action_server(std::chrono::seconds(100));
