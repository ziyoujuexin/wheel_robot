...
/** AIUI C++接口V2版本，是对AIUI_C.h中接口的封装。建议使用该版本 **/
 ...
 * @param data
 */
void AIUIMessageCallbackFunc(AIUIEvent event, void* data);
...
/**
 * 数据捆绑类，支持int、long、string和Buffer*类型数据按名存取。
 */
class IDataBundle
{
    friend class IAIUIEvent;
    friend void AIUIMessageCallbackFunc(AIUIEvent event, void* data);

private:
    AIUIDataBundle mBundle;

public:
    /**
     * 按key获取字符串，若key不存在则返回defaultVal。
     *
	 ...
	 */
    AIUIEXPORT const char* getString(const char* key, const char* defaultVal) const;

    /**
     * 按key获取二进制缓存，若key不存在则返回nullptr。
     *
	 * 注：getBinary返回地址对应的内存由IDataBundle管理，不可（不需要）在外部delete或者free。
     *
     * @param key 名称
     * @param dataLen [OUT]用于返回数据长度
	 */
    AIUIEXPORT const char* getBinary(const char* key, int* dataLen) const;

    /**
     * 按key获取long值，若key不存在则返回defaultVal。
     *
     * @param key
     * @param defaultVal
     * @return
     */
    AIUIEXPORT long getLong(const char* key, long defaultVal) const;

    /**
     * 按key获取int值，若key不存在则返回defaultVal。
     *
     * @param key
     * @param defaultVal
     * @return
     */
    AIUIEXPORT int getInt(const char* key, int defaultVal) const;
};
...
public:
	 ...
	 */
    AIUIEXPORT int getEventType() const;

    /**
	 * 获取扩展参数1。
	 */
    AIUIEXPORT int getArg1() const;

    /**
	 * 获取扩展参数2。
	 */
    AIUIEXPORT int getArg2() const;

    /**
	 * 获取描述信息。返回的指针不可外部delete或者free。
	 */
    AIUIEXPORT const char* getInfo() const;

    /**
	 * 获取附带数据。返回的内存不可外部delete或者free。
	 */
    AIUIEXPORT const IDataBundle* getData() const;
};
...
public:
    ...
    AIUIEXPORT static IAIUIMessage* create(
        int msgType, int arg1 = 0, int arg2 = 0, const char* params = "", AIUIBuffer data = nullptr);
 ...
 */
class AIUIEXPORT IAIUIListener
{
public:
    /**
...
public:
	 ...
	 */
    AIUIEXPORT static IAIUIAgent AIUI_DEPRECATED* createAgent(const char* params,
                                                              IAIUIListener* listener);
	 ...
	 * @return AIUIAgent 单例对象指针
	 */
    AIUIEXPORT static IAIUIAgent* createAgent(const char* params,
                                              IAIUIListener* listener,
                                              const char* sn);
	 ...
	 */
    AIUIEXPORT void sendMessage(const IAIUIMessage* message);
...
public:
	 ...
	 * @param szDir 目录（windows下以"\\"结尾，其他平台以"/"结尾）
	 */
    AIUIEXPORT static bool setRawAudioDir(const char* szDir);
     ...
     *
     */
    AIUIEXPORT static int getSystemInfo(const char* key, char* out, int maxSize);

    /**
     * 设置GPS坐标。参数优先级：CMD_WRITE消息params参数中携带 > aiui.cfg的audioparams中设置 > setGpsPos方法设置。
     *
     * @param lng 经度
     * @param lat 纬度
     */
    AIUIEXPORT static void setGpsPos(float lng, float lat);
};
